...
export default function FacultyDashboard({ user, onLogout, isDarkMode, onToggleTheme }) {
  ...
  const renderStatusBadge = (status) => {
    switch (status) {
      case "approved":
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5 w-fit">
            <CheckCircle className="h-3.5 w-3.5" /> Authorized by Principal (QR Pass Ready)
          </span>
        );
      case "pending_hod":
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 flex items-center gap-1.5 w-fit">
            <Clock className="h-3.5 w-3.5 animate-pulse" /> Step 1: Pending HOD Clearance
          </span>
        );
      case "pending_principal":
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 flex items-center gap-1.5 w-fit">
            <Clock className="h-3.5 w-3.5 animate-pulse" /> Step 2: Cleared by HOD • Pending Principal Sign-Off
          </span>
        );
      case "rejected":
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-red-500/20 text-red-700 dark:text-red-400 border border-red-500/30 flex items-center gap-1.5 w-fit">
            <XCircle className="h-3.5 w-3.5" /> Rejected by Principal
          </span>
        );
      case "exited":
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-sky-500/20 text-sky-700 dark:text-sky-300 border border-sky-500/30 flex items-center gap-1.5 w-fit">
            <LogOut className="h-3.5 w-3.5" /> Currently Off-Campus (Left)
          </span>
        );
      case "closed":
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 flex items-center gap-1.5 w-fit">
            <CheckCircle className="h-3.5 w-3.5" /> Returned & Pass Closed
          </span>
        );
      default:
        return (
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700">
            {status}
          </span>
        );
    }
  ...
  const pendingCount = myPasses.filter((p) => p.status === "pending_principal" || p.status === "pending_hod").length;
    ...
    <div className="min-h-screen bg-[#f0f5fa] dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans pb-16 transition-colors">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 space-y-8">
        ...
        {activeTab === "apply" && (
          <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
            <div className="flex items-center gap-3 pb-4 border-b border-slate-200 dark:border-slate-800">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Faculty Gate Pass Application Form</h2>
            ...
            {formErr && (
              <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-300 text-xs font-bold flex items-center gap-3">
            ...
            <form onSubmit={handleApply} className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
              <div className="md:col-span-2 space-y-1.5">
                <label className="block font-semibold text-slate-700 dark:text-slate-200">
                  Reason for Outing / Leaving Campus <span className="text-amber-500">*</span>
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500/60 font-medium"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block font-semibold text-slate-700 dark:text-slate-200">
                    Exit Date <span className="text-amber-500">*</span>
                  </label>
                  <input
                    type="date"
                    required
                    ...
                    className="w-full p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white focus:outline-none focus:border-amber-500/60 font-medium"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="block font-semibold text-slate-700 dark:text-slate-200">
                    Exit Time <span className="text-amber-500">*</span>
                  </label>
                  <input
                    type="time"
                    required
              ...
              <div className="md:col-span-2 pt-4 flex justify-end">
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-8 py-3.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-extrabold text-xs shadow-lg shadow-amber-500/20 transition-all cursor-pointer flex items-center gap-2"
                >
                  {submitting ? (
        ...
        {activeTab === "late_mark" && (
          <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
            <div className="flex items-center gap-3 pb-4 border-b border-slate-200 dark:border-slate-800">
              <div className="p-3 rounded-2xl bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border border-indigo-500/30">
                <Clock className="h-6 w-6" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Teacher Staff Late Mark Form</h2>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
        ...
        {/* Tab 3: My Gate Passes */}
        {activeTab === "passes" && (
          <div className="space-y-4">
            {myPasses.length === 0 ? (
              <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl shadow-sm">
                <FileText className="h-12 w-12 text-slate-400 dark:text-slate-600 mx-auto mb-3" />
                <h3 className="text-base font-bold text-slate-900 dark:text-white">No Gate Pass Records Found</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {myPasses.map((pass) => (
                  <div
                    key={pass.id}
                    className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all shadow-sm flex flex-col justify-between"
                  >
                    <div>
                    ...
                    {pass.qr_code && pass.status !== "exited" && (
                      <div className="pt-4">
                        <button
                          ...
                          className="w-full py-3 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-extrabold text-xs flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 transition-all cursor-pointer"
                        >
      ...
      {selectedPass && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-sm p-6 shadow-2xl text-center space-y-4 relative">
            <button
            ...
            <div className="text-left bg-slate-50 dark:bg-slate-950 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs space-y-1.5">
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Faculty Name:</span>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-amber-600 dark:text-amber-300 font-semibold">{selectedPass.reason}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Exit Window:</span>
                <span className="text-slate-800 dark:text-slate-300 font-mono">
