...
export default function TeacherDashboard({ user, onLogout }) {
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
          <div>
            ...
            <p className="text-xs text-slate-500 font-bold mt-1 max-w-2xl">
              {activeTab === "gatepasses" ? "Review and approve Outing applications. Upon your approval, they are forwarded to the HOD, and parent SMS notification will dispatch at final HOD clearance." : "Monitor first-lecture late-comers, verify parent contact details, and dispatch automatic SMS alerts to prevent proxy numbers."}
            </p>
          </div>
          <button
    onClick={fetchData}
  ...
  >
    ...
    /* Dynamic KPIs / Analytics Grid */
        ...
        {activeTab === "late" && <div className="grid grid-cols-1 md:grid-cols-3 gap-5 animate-fade-in">
  ...
  >
    ...
    return <tr key={entry.id} className="hover:bg-slate-50/50 transition">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-center">
                              {isSmsSent ? <span className="inline-flex items-center space-x-1 px-2.5 py-1 text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-lg">
      ...
      className={`inline-flex items-center space-x-1.5 px-3 py-1.5 text-xs font-bold rounded-lg border shadow-sm transition-all cursor-pointer ${!entry.parent_phone ? "bg-slate-50 text-slate-400 border-slate-200 cursor-not-allowed" : "bg-white hover:bg-slate-50 text-slate-800 border-slate-200 active:scale-95"}`}
    >
    ...
    /* Broadcast Logs session center */
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
              ...
              {smsLogs.length === 0 ? <div className="py-8 text-center text-slate-400">
                  ...
                  {smsLogs.map((log) => <div key={log.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex justify-between items-start">
