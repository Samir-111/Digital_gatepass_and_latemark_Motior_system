...
import { 
  FileText, Calendar, Compass, Phone, User, Clock, AlertTriangle, CheckCircle, 
  XCircle, ArrowUpRight, LogOut, Download, Plus, Trash2, ShieldAlert, Sparkles, RefreshCw
} from 'lucide-react';
...
export default function StudentDashboard({ user, onLogout }: StudentDashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<'status' | 'apply' | 'history' | 'profile' | 'late'>('status');

  // Late come state variables
  const [lateEntries, setLateEntries] = useState<any[]>([]);
  const [lateReason, setLateReason] = useState('');
  const [lateTime, setLateTime] = useState('');
  const [submittingLate, setSubmittingLate] = useState(false);

  // Cancel confirmation & toast modal state
  ...
  const handleLateComeSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleApply = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!reason || !exitTime) {
      showToast('Kripya saare mandatory fields (Reason aur Leaving Date & Time) ko sahi se fill karein!', 'error');
      return;
    }
  ...
  const getRiskBadge = (level?: GatePass['risk_level']) => {
    ...
    <div className="min-h-screen bg-slate-100 pb-12 font-sans">
          ...
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2 text-sm text-slate-700">
                {photo ? (
                  <img src={photo} alt={user.name} className="h-8 w-8 rounded-full border border-slate-200 object-cover" />
                ) : (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center space-x-4">
            {photo ? (
              <img src={photo} alt={user.name} className="h-16 w-16 rounded-2xl border border-slate-200 object-cover shadow-sm" />
            ) : (
          ...
          <div className="flex gap-2 shrink-0 overflow-x-auto pb-1 md:pb-0">
            ...
            <button 
              onClick={() => { setActiveTab('late'); fetchLateEntries(); }}
        ...
        {activeTab === 'status' && (
          <div className="space-y-6">
            {loading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col items-center justify-center text-center">
                  ...
                  {activePass.qr_code ? (
                    ...
                    <div className="bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 h-52 w-52 flex flex-col items-center justify-center mb-4 p-4 text-slate-400">
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  {activePass.status === 'exited' && (
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex items-start space-x-3 text-indigo-800">
              ...
              <div className="text-center py-12 bg-white rounded-2xl border border-slate-200 shadow-sm p-8 max-w-lg mx-auto">
        ...
        {activeTab === 'apply' && (
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm max-w-2xl mx-auto">
            ...
            <form onSubmit={handleApply} className="space-y-5">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Leaving Date & Time</label>
                <input
                   required
                   type="datetime-local"
                   value={exitTime}
            ...
            {passes.length > 0 && (
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {(Object.entries(
                    passes.reduce((acc: Record<string, { total: number; closed: number; pending: number; approved: number; dates: string[] }>, pass) => {
                  ...
                  ) as Array<[string, { total: number; closed: number; pending: number; approved: number; dates: string[] }]>).map(([month, stat]) => (
                    <div key={month} className="p-4 rounded-xl bg-slate-50 border border-slate-200 shadow-inner flex flex-col justify-between">
                      <div>
                      ...
                      <div className="border-t border-slate-200 pt-2 mt-2">
                        ...
                        <div className="text-[10px] text-slate-600 font-medium flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                          {stat.dates.map((d, idx) => (
                            <span key={idx} className="bg-white border border-slate-200 px-1.5 py-0.5 rounded text-slate-700">{d}</span>
                          ))}
                        </div>
                      </div>
                    </div>
        ...
        {activeTab === 'profile' && (
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm max-w-xl mx-auto">
            ...
            {profileSuccess && (
              <div className="mb-4 bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded-r-lg text-xs font-semibold text-emerald-700">
            ...
            <form onSubmit={handleProfileUpdate} className="space-y-5">
              <div className="flex flex-col items-center justify-center space-y-2 mb-4">
                {photo ? (
                  <img src={photo} alt={user.name} className="h-24 w-24 rounded-full border-2 border-slate-200 object-cover shadow" />
                ) : (
                  <div className="h-24 w-24 rounded-full bg-slate-100 border-2 border-slate-200 flex items-center justify-center">
              ...
              {lateEntries.length === 0 ? (
                ...
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-1">
                  {lateEntries.map((entry) => (
                    <div key={entry.id} className="p-4 bg-slate-50 hover:bg-slate-100/80 rounded-xl border border-slate-200/60 transition flex flex-col sm:flex-row justify-between gap-4">
                      <div className="space-y-1 flex-1">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-[10px] text-slate-400 font-bold">
                            Logged on: {new Date(entry.created_at).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}
                          </span>
                        </div>
