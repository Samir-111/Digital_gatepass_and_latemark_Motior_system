...
export default function Login({ onLoginSuccess }: LoginProps) {
  ...
  // Forgot Password: Step 1 - Request OTP Handler
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.dev_otp) {
        successMessage = `🔧 [Development Mode]: OTP code is ${res.dev_otp} (SMTP is not configured, code shown for testing).`;
      }
  ...
  // Forgot Password: Step 2 - Verify OTP & Reset Password Handler
  ...
  return (
    <div className="relative min-h-screen bg-slate-100 dark:bg-slate-950 flex flex-col justify-center py-10 sm:px-6 lg:px-8 font-sans overflow-hidden transition-colors duration-300">
      ...
      <div className="relative z-10 mt-6 sm:mx-auto w-full sm:max-w-md px-4">
        <div className="bg-white dark:bg-slate-900 py-8 px-6 shadow-xl rounded-2xl border border-slate-200 dark:border-slate-800 sm:px-10">
          ...
          {/* Status Alert Panels */}
          {error && (
            <div className="mb-5 bg-red-50 dark:bg-red-950/30 border-l-4 border-red-500 p-4 rounded-r-xl flex items-start space-x-3 shadow-sm">
          ...
          {successMsg && (
            <div className="mb-5 bg-emerald-50 dark:bg-emerald-950/30 border-l-4 border-emerald-500 p-4 rounded-r-xl flex items-start space-x-3 shadow-sm">
          ...
          {!isRegistering && !isForgotPassword ? (
            <div className="space-y-6">
              ...
              <div className="mt-6">
                ...
                {showHODDropdown && (
                  <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl space-y-3 animation-fade-in">
                    <div className="flex justify-between items-center">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-emerald-50 dark:bg-emerald-955/30 border border-emerald-200 dark:border-emerald-900 rounded-xl p-3 flex items-start space-x-2">
                ...
                <span className="text-[10px] text-emerald-800 dark:text-emerald-300 font-bold leading-relaxed">
                  {otpSent 
                    ? 'Enter the 6-digit OTP code sent to your email, then set and confirm your new account password.' 
                    : 'Provide your registered institutional email to receive a secure 6-digit verification code.'}
                </span>
              </div>
