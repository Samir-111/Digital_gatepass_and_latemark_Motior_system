...
export default function AdminDashboard({ user, onLogout }) {
  ...
  >
    ...
    /* TAB 1: OVERVIEW ANALYTICS */
          ...
          {activeTab === "analytics" && <div className="space-y-8">
    ...
    /* Custom Styled Department Requests chart */
                ...
                <div className="border border-slate-100 rounded-2xl p-5 bg-slate-50/30">
                  ...
                  <div className="space-y-3">
                    {deptList.map((dept) => {
    const count = studentList.filter((s) => s.department === dept.department_name).length * 2;
    const totalCount = studentList.length * 2 || 1;
    const pct = Math.min(100, Math.round(count / totalCount * 100)) || 10;
    return <div key={dept.id} className="space-y-1">
                    ...
                    {deptList.length === 0 && <div className="text-center py-6 text-xs text-slate-400">No departments configured yet.</div>}
                  </div>
                </div>
  ...
  ].map((item, idx) => <div key={idx} className="flex items-center justify-between text-xs font-bold text-slate-700">
  ...
  >
    ...
    /* TAB: GATE PASS TRAFFIC CONSOLE */
          ...
          {activeTab === "gatepasses" && <div className="space-y-6">
  ...
  >
                    ...
                    {deptList.map((dept) => <option key={dept.id} value={dept.department_name}>{dept.department_name}</option>)}
                  </select>
                </div>
  ...
  >
    ...
    return <div className="space-y-4">
      ...
      return <tr key={pass.id} className="hover:bg-slate-50/40">
                                ...
                                <td className="px-5 py-4 whitespace-nowrap">
                                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${badgeColors[pass.status] || "bg-slate-50"}`}>
                                    {pass.status.toUpperCase()}
                                  </span>
                                  {pass.risk_level && <div className="mt-1">
                                      <span className={`inline-flex items-center text-[9px] font-black uppercase px-1.5 py-0.5 rounded border ${pass.risk_level === "high" ? "bg-red-50 text-red-700 border-red-200" : pass.risk_level === "medium" ? "bg-amber-50 text-amber-600 border-amber-200" : "bg-blue-50 text-blue-600 border-blue-200"}`}>
                                        USAGE RISK: {pass.risk_level}
                                      </span>
                                    </div>}
                                </td>
                                <td className="px-5 py-4 whitespace-nowrap">
                                  {hasSmsFired ? <div className="space-y-1">
    ...
    /* Student records list */
              ...
              <div className="overflow-x-auto border border-slate-200 rounded-2xl">
                <table className="min-w-full divide-y divide-slate-100 text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-slate-100 text-slate-700">
                    {studentList.map((stud) => <tr key={stud.id} className="hover:bg-slate-50/50">
    ...
    /* TAB 3: MANAGE HODS */
          ...
          {activeTab === "hods" && <div className="space-y-6">
              ...
              <div className="overflow-x-auto border border-slate-200 rounded-2xl">
                <table className="min-w-full divide-y divide-slate-100 text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-slate-100 text-slate-700">
                    {hodList.map((hod) => <tr key={hod.id} className="hover:bg-slate-50/50">
          ...
          {activeTab === "teachers" && <div className="space-y-6">
              ...
              <div className="overflow-x-auto border border-slate-200 rounded-2xl">
                <table className="min-w-full divide-y divide-slate-100 text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-slate-100 text-slate-700">
                    {teacherList.map((teacher) => <tr key={teacher.id} className="hover:bg-slate-50/50">
    ...
    /* TAB 4: MANAGE GUARDS */
          ...
          {activeTab === "guards" && <div className="space-y-6">
              ...
              <div className="overflow-x-auto border border-slate-200 rounded-2xl">
                <table className="min-w-full divide-y divide-slate-100 text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-slate-100 text-slate-700">
                    {guardList.map((g) => <tr key={g.id} className="hover:bg-slate-50/50">
    ...
    /* TAB 5: MANAGE DEPARTMENTS */
          ...
          {activeTab === "depts" && <div className="space-y-6 max-w-xl">
  ...
  />
              ...
              <div className="overflow-x-auto border border-slate-200 rounded-2xl">
                <table className="min-w-full divide-y divide-slate-100 text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-slate-100 text-slate-700">
                    {deptList.map((dept) => <tr key={dept.id} className="hover:bg-slate-50/50">
    ...
    /* TAB: PARENT PHONE MAPPINGS DIRECTORY */
          ...
          {activeTab === "parent_contacts" && <div className="space-y-8 animate-in fade-in duration-200">
  ...
  >
    ...
    /* Preview Parsed Mapping Column */
                ...
                <div className="border border-slate-200 rounded-2xl p-5 bg-white space-y-4 flex flex-col min-h-[300px]">
                  ...
                  {parentContactsPreview.length > 0 ? <div className="flex-1 flex flex-col justify-between">
                      <div className="overflow-y-auto max-h-[220px] border border-slate-100 rounded-xl divide-y divide-slate-50">
                        {parentContactsPreview.map((item, idx) => <div key={item.id || idx} className="flex justify-between items-center p-2.5 text-xs">
                            <div>
  ...
  >
                    ...
                    </div> : <div className="flex-1 flex flex-col justify-center items-center text-center p-6 text-slate-400">
  ...
  />
                ...
                <div className="overflow-x-auto border border-slate-200 rounded-2xl shadow-sm">
                  <table className="min-w-full divide-y divide-slate-100 text-xs">
                    ...
                    <tbody className="bg-white divide-y divide-slate-100 text-slate-700">
                      {parentContacts.filter((item) => {
    const query = parentSearchQuery.toLowerCase();
    return (item.roll_no || "").toLowerCase().includes(query) || (item.name || "").toLowerCase().includes(query) || (item.parent_phone || "").toLowerCase().includes(query);
  }).map((item, idx) => <tr key={idx} className="hover:bg-slate-50/50">
    ...
    /* TAB 6: AUDIT ACTIVITY LOGS */
          ...
          {activeTab === "logs" && <div className="space-y-4">
              ...
              <div className="bg-slate-900 text-slate-300 font-mono text-[11px] rounded-2xl p-5 overflow-y-auto max-h-[500px] shadow-inner space-y-2 border border-slate-800">
                {logs.map((log) => <div key={log.id} className="flex items-start space-x-2 leading-relaxed">
    ...
    /* Custom Delete Confirmation Modal */
        ...
        {deleteConfirm && <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
