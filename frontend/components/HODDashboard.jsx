...
export default function HODDashboard({ user, onLogout }) {
  ...
  const getStatusBadge = (status) => {
    ...
    return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${styles[status]}`}>
        {labels[status]}
      </span>;
  };
  const getRiskIndicator = (level) => {
    ...
    return <span className={`inline-flex items-center space-x-1 px-2.5 py-1 rounded-lg text-xs font-bold border ${styles[level]}`}>
  ...
  >
    ...
    /* Filter controls, only visible in Pending tab */
            ...
            {activeTab === "pending" && <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
  ...
  >
    ...
    /* TAB 1: PENDING QUEUE */
          ...
          {activeTab === "pending" && <div className="p-6">
              {loading ? <div className="text-center py-12">
                ...
                </div> : filteredPending.length === 0 ? <div className="text-center py-12 text-slate-400">
                  ...
                  {filteredPending.map((pass) => <div key={pass.id} className="border border-slate-200 shadow-sm rounded-2xl bg-white p-5 hover:border-slate-300 transition-all">
                      {
  ...
  >
    ...
    /* TAB 3: LATE COMERS REGISTER */
          ...
          {activeTab === "late_come" && <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/60">
                  ...
                  <div className="text-xl font-black text-amber-600 mt-0.5">
                    {(() => {
    const currentMonth = (/* @__PURE__ */ new Date()).toLocaleString("en-US", { month: "long", year: "numeric" });
    ...
    /* Grouped Monthwise List */
              ...
              {lateEntries.length === 0 ? <div className="text-center py-12 text-slate-400">
    ...
    return sortedMonths.map((month) => <div key={month} className="space-y-3">
                        ...
                        <div className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
                          <table className="min-w-full divide-y divide-slate-100">
                            ...
                            <tbody className="bg-white divide-y divide-slate-100 text-xs text-slate-700">
                              {groups[month].map((entry) => <tr key={entry.id} className="hover:bg-slate-50/30">
