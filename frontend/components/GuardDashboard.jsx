...
export default function GuardDashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    fetchGuardData();
    const interval = setInterval(fetchGuardData, 1e4);
    startScanning();
    return () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border capitalize ${styles[status]}`}>
        {status}
      </span>;
  };
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center space-x-2">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold border uppercase tracking-wider ${scannerActive ? "bg-emerald-50 text-emerald-700 border-emerald-200 animate-pulse" : "bg-slate-100 text-slate-500 border-slate-200"}`}>
  ...
  />
    ...
    /* Overlaid UI frame when camera is live */
                ...
                {scannerActive && <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-10">
    ...
    /* Screen when scanner is disabled or failed */
                ...
                {!scannerActive && <div className="absolute inset-0 bg-slate-900/95 p-6 text-center z-20 flex flex-col items-center justify-center">
                    {scannerError ? <div className="space-y-3">
  ...
  >
    ...
    /* Camera selection dropdown & control switches */
              ...
              {availableCameras.length > 1 && <div className="mb-4">
  ...
  >
                    {availableCameras.map((cam) => <option key={cam.id} value={cam.id}>
  ...
  />
                ...
                {imageUploadError && <p className="mt-2 text-[11px] font-medium text-rose-500 flex items-start space-x-1">
  ...
  >
    ...
    /* Result Header Box */
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-center min-h-[460px]">
              ...
              {!verifiedPass && !verificationError && !verifyLoading && <div className="text-center py-12 text-slate-400">
              ...
              {verifyLoading && <div className="text-center py-12">
    ...
    /* SECURITY ERROR FLAGS */
              ...
              {verificationError && <div className="space-y-4">
                  <div className={`p-5 rounded-2xl border flex items-start space-x-3.5 ${duplicateWarning ? "bg-red-50 border-red-100 text-red-800" : "bg-rose-50 border-rose-100 text-rose-800"}`}>
                    ...
                    <div>
                      ...
                      {duplicateWarning && <div className="mt-3 bg-red-100 border border-red-200 rounded-lg p-2.5 text-[10px] font-bold text-red-800 flex items-center space-x-1.5">
    ...
    /* Actions depending on pass status */
                  ...
                  <div className="border-t border-slate-200 pt-4 flex gap-3">
                    {verifiedPass.status === "approved" && <button
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-100">
              ...
              <tbody className="bg-white divide-y divide-slate-100 text-xs text-slate-700">
                {todayEntries.map((pass) => <tr key={pass.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-slate-600 font-semibold">
                      {pass.exit_marked_at ? <span className="flex items-center text-emerald-600 font-bold">
                          <CheckCircle className="h-3.5 w-3.5 mr-1" />
                          {new Date(pass.exit_marked_at).toLocaleTimeString()}
                        </span> : <span className="text-slate-400">No Scan Logged</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {getStatusBadge(pass.status)}
                    </td>
                  </tr>)}
                {todayEntries.length === 0 && <tr>
